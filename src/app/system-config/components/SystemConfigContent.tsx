      ...
      {hasTemplates && (
        <div className="space-y-3">
          {textareaFields.map((field) => {
            const isOpen = expandedGroups[field.key] !== false; // open by default
            return (
        ...
        <div className="flex items-center gap-3 mb-1">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Configure bank details, registry URLs, email templates, thresholds, and retention policies
            </p>
          </div>
        </div>
      </div>
